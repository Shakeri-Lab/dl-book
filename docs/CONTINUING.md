...
> structural contracts may differ across platforms. Both notebook workflows declare a
> one-thread numerical-library default to reduce scheduling-dependent drift; the
> publication gate records and verifies its initial state. Deliberately heavier chapter
> cells may still choose their own PyTorch thread count, and the output gates remain
> authoritative. The Preface, Epilogue, and two non-executable appendices retain honest
> unavailable placeholders; executed copies remain CI evidence rather than public
> downloads.
