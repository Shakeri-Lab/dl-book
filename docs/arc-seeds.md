...
| Each continuation belongs to its prefix; width counts alternatives, depth counts token positions; candidates are alternatives, not consecutive tokens | ch. 11 optional greedy/beam replay (September 13 approved revision) | future multi-token prediction and speculative-decoding discussion: prefix dependence, candidate-tree geometry, then a separately defined verification contract; see `docs/greedy-tree-excerpt.md` | publication approved; harvest not yet authored |
