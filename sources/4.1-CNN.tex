...
\begin{lstlisting}[language=Python, caption=Demonstrating padding effects from the textbook]
import torch
from torch import nn

# Create a simple 2D convolution layer with different padding
conv_no_pad = nn.Conv2d(1, 1, kernel_size=3, padding=0)  # No padding
conv_with_pad = nn.Conv2d(1, 1, kernel_size=3, padding=1)  # Padding of 1

# Create a sample 8x8 input
X = torch.randn(1, 1, 8, 8)

# Apply convolutions
output_no_pad = conv_no_pad(X)
output_with_pad = conv_with_pad(X)

print(f"Input shape: {X.shape}")
print(f"Output without padding: {output_no_pad.shape}")  # Will be 6x6
print(f"Output with padding: {output_with_pad.shape}")    # Will be 8x8
\end{lstlisting}
...
Let's explore strides with code from the textbook:

\begin{lstlisting}[language=Python, caption=Demonstrating stride effects]
...
print(f"Input shape: {X.shape}")
print(f"Output with stride=2: {output_stride2.shape}")  # Will be 4x4

# We can compute output size with formula:
...
Let's implement this from the textbook:

\begin{lstlisting}[language=Python, caption=Multiple input and output channels]
import torch
import torch.nn as nn

# Define a custom multi-channel convolution
def corr2d_multi_in(X, K):
    # Iterate through the channels of X and K, perform cross-correlation
    # and add the results
    return sum(d2l.corr2d(x, k) for x, k in zip(X, K))

# Multiple output channels
def corr2d_multi_in_out(X, K):
    # Iterate through the 0th dimension of K (output channels)
    return torch.stack([corr2d_multi_in(X, k) for k in K], 0)

# Using PyTorch's built-in layers
conv_multi = nn.Conv2d(3, 16, kernel_size=3, padding=1)  # 3 input, 16 output channels

# Test with RGB input
X_rgb = torch.randn(1, 3, 32, 32)  # Batch=1, Channels=3, Height=32, Width=32
output = conv_multi(X_rgb)
print(f"Input shape: {X_rgb.shape}")
print(f"Output shape: {output.shape}")  # Will be [1, 16, 32, 32]
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Pooling operations from the textbook]
# Define pooling layers
pool2d = nn.MaxPool2d(3)  # Default stride = kernel_size = 3
...
\begin{lstlisting}[language=Python, caption=LeNet-5 implementation from the textbook]
import torch.nn as nn
...
class LeNet(nn.Module):
    ...
    def forward(self, x):
        # First conv block: conv -> activation -> pool
...
\begin{lstlisting}[language=Python, caption=AlexNet implementation from the textbook]
class AlexNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),  # ReLU instead of sigmoid!
            ...
            # Three more conv layers without pooling
...
\begin{lstlisting}[language=Python, caption=VGG blocks from the textbook]
def vgg_block(num_convs, out_channels):
    ``""A VGG block consists of multiple 3x3 convs followed by max pooling"""
    layers = []
...
class VGG(nn.Module):
    ...
    def _make_layers(self, arch):
        ...
        for (num_convs, out_channels) in arch:
            layers.append(vgg_block(num_convs, out_channels))
        return nn.Sequential(*layers)

# VGG-11 configuration: (num_convs, out_channels) per block
...
\begin{lstlisting}[language=Python, caption=NiN blocks with 1×1 convolutions]
def nin_block(out_channels, kernel_size, strides, padding):
    ``""NiN block: regular conv followed by two 1x1 convs"""
    return nn.Sequential(
        nn.LazyConv2d(out_channels, kernel_size, strides, padding),
        nn.ReLU(),
        nn.LazyConv2d(out_channels, kernel_size=1),  # 1x1 conv!
        nn.ReLU(),
        nn.LazyConv2d(out_channels, kernel_size=1),  # Another 1x1
        nn.ReLU()
    )
...
class NiN(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            ...
            nn.MaxPool2d(3, stride=2),
            nn.Dropout(0.5),
            # Last NiN block has num_classes channels
    ...
    def forward(self, x):
        x = self.features(x)
        x = self.pool(x)
        return x.squeeze()  # Remove spatial dimensions
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Inception module from the textbook]
...
\begin{lstlisting}[language=Python, caption=Residual blocks from the textbook]
...
\begin{lstlisting}[language=Python, caption=Depthwise separable convolution]
...
# Standard 3x3 conv: in_channels * out_channels * 3 * 3
# Depthwise separable: in_channels * 3 * 3 + in_channels * out_channels
# Reduction factor: ~9x for large channel counts!
\end{lstlisting}
