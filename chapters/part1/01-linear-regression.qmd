...
![Why the penalties behave differently: minimizing the loss subject to a weight budget. The spherical $L_2$ budget shrinks the solution toward the origin; the diamond-shaped $L_1$ budget has corners on the axes, so the solution tends to land where some coordinates are exactly zero (Exercise 5).](../../figures/generated/1_1_lin_Recap-fig09.svg){#fig-regularization width=55%}
