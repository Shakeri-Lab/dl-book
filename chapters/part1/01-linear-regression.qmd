...
$$
\hat y = \parameterpart{\vect{w}^\top}\featurepart{\vect{x}} + b .
$$

The second term, $b$, is the model's default prediction when the input carries no
relevant information. The first term, $\vect{w}^\top\vect{x}$, adjusts that baseline
up or down based on how the input matches the weight vector.

![The dot product measures the signed shadow of an input along the weight vector. Aligned inputs raise the prediction above $b$, orthogonal inputs leave it at $b$, and opposing inputs lower it.](../../figures/generated/1_1_linear_response.svg){#fig-linear-response width=88% fig-alt="A two-dimensional weight vector is shown with aligned, orthogonal, and opposing input vectors. Their dot products are positive, zero, and negative, so their predictions lie above, at, and below the bias baseline."}
...
therefore a learned *pattern scorer*: $\vect{w}$ is a template whose direction and
scale both matter. In Part II, an image filter will slide this same dot product across
an image. In Part IV, attention will build learned similarities between queries and
keys from the same primitive.

For compact matrix notation, we will absorb $b$ into the weights.\footnote{Append a
column of ones to $\matr{X}$ and append $b$ to $\vect{w}$. This is often called
\emph{bias augmentation} or the use of \emph{homogeneous coordinates}. It is
bookkeeping, not a
new learning principle. A formula or software layer without an explicit bias may
instead be deliberately constraining $b=0$.} Then

$$
\hat y = \vect{w}^\top \vect{x} \qquad \text{(bias included)}.
$$
...
- **Full column rank.** The minimizer is unique, and only then may we write
  $\hat{\vect{w}}=(\matr{X}^{\top}\matr{X})^{-1}\matr{X}^{\top}\vect{y}$.
- **Rank deficiency.** The fitted prediction is still a unique projection, but the
  ...
  $\hat{\vect{w}}_{\min}=\matr{X}^{+}\vect{y}$, where $\matr{X}^{+}$ is the
  Moore--Penrose pseudoinverse. This matters in deep learning because $d>n$ is common,
  not exceptional.
- **Conditioning.** Forming the normal equations squares the spectral condition number:
  $\kappa_2(\matr{X}^{\top}\matr{X})=\kappa_2(\matr{X})^2$. QR-, SVD-, or
  library-based least-squares routines are the computational default.
- **Cost.** For a tall dense matrix, forming $\matr{X}^{\top}\matr{X}$ costs
  $O(nd^2)$, followed by about $O(d^3)$ for factorization. Either term may dominate.
...
It must therefore live in the **column space** of $\matr{X}$. If
$\targetpart{\vect{y}}$ lies outside that space, the best prediction is its projection.
The residual $\vect{e}=\targetpart{\vect{y}}-\predictionpart{\hat{\vect{y}}}$ is
...
Each fitted value is a weighted combination of the training targets. Part IV will turn
this same idea into attention by choosing mixing weights through similarity.
...
# [1] create the problem and evaluate its loss surface
...
# [3] show perspective and overhead views
...
Enough theory. Let us implement linear regression in PyTorch, first with the algebra
...
# [1] establish reproducible state
torch.manual_seed(6050)
```
::::
...
# [2] generate data and check shapes
...
#| label: closed-form
#| code-fold: false
# [1] append the constant feature
X_aug = torch.cat([X, torch.ones(X.shape[0], 1)], dim=1)

# [2] solve without forming the normal equations
w_ols = torch.linalg.lstsq(X_aug, y).solution

# [3] compare with the known truth
...
#| code-fold: false
# [1] store parameters and define predictions
class LinearRegressionScratch:
    ...
    def step(self, X: torch.Tensor, y: torch.Tensor) -> float:
        # [2] convert batch residuals into the MSE update
...
# [4] compare the learned parameters
...
# [1] pair the module, objective, and optimizer
net = nn.Linear(in_features=2, out_features=1)
optimizer = torch.optim.SGD(net.parameters(), lr=0.1)
loss_fn = nn.MSELoss()
nn_losses = []

# [2] repeat forward, backward, and update
for _ in range(100):
...
#| code-fold: false
# [1] collect losses on a common scale
closed_losses = [
    float((y ** 2).mean()),
...
# [2] evaluate predictions on the slice
...
#| code-fold: false
# [1] create a high-variance problem
