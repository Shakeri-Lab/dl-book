...
![The normal equations, geometrically: the optimal prediction $\hat{\vect{y}} = \matr{X}\hat{\vect{w}}$ is the projection of $\vect{y}$ onto the column space of $\matr{X}$; the residual $\vect{e}$ is orthogonal to it.](../../figures/generated/1_1_lin_Recap-fig06.svg){#fig-projection width=62%}
...
![The classical picture: as model complexity grows, bias falls while variance rises; total error is U-shaped, with sweet spots between under- and overfitting.](../../figures/generated/1_1_lin_Recap-fig07.svg){#fig-bias-variance width=70%}
...
![Why the penalties behave differently: minimizing the loss subject to a weight budget. The spherical $L_2$ budget shrinks the solution toward the origin; the diamond-shaped $L_1$ budget has corners on the axes, so the solution tends to land where some coordinates are exactly zero (Exercise 6).](../../figures/generated/1_1_lin_Recap-fig09.svg){#fig-regularization width=55%}
