...
optimum = torch.linalg.lstsq(A, y1, driver="gelsd").solution
...
:::: {.plan-code}
::: {.plan}
1. Follow repeated gradient steps on one quadratic from a fixed start.
2. Compare loss trajectories for three learning rates under the same step budget.
:::
...
**Case 1 — decomposable objectives.** When each prediction is computed without using
the other examples in its batch, the loss can be a mean of fixed per-example terms,
$\loss(\vect{w}) = \E_{x}[\ell(x; \vect{w})]$. At fixed parameters, a uniformly
sampled minibatch average is an unbiased estimator of that objective, and its
gradient an unbiased estimator of the full gradient. Under this contract, $B$ is a
compute-and-noise dial, not a correctness dial. Cross-entropy and squared error fit
this case for batch-independent predictors. A batch-dependent forward pass changes
the contract even if the last line still averages cross-entropy; Chapter 9 supplies
an example.
...
:::: {.plan-code .plan-code-wide}
::: {.plan}
1. Create a regression problem with differently scaled feature directions.
2. Run the same loss through SGD, momentum, and Adam using matched batches.
3. Compare progress under the shared step budget.
:::
...
:::: {.plan-code}
::: {.plan}
1. Drop independent activations and rescale the survivors.
2. Choose a fixed activation batch and retention probability.
3. Compare training-time averages with unchanged evaluation outputs.
:::
...
comparisons lean on that. Read the loss line with its contract: the simple
unbiasedness argument applies to batch-independent predictions at fixed parameters
under uniform sampling (@sec-04-estimator-cases). The earlier dependence caveat for
random reshuffling still applies to this training trajectory.
