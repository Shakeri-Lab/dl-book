...
set of refinements (step size, minibatch noise, momentum, and adaptive per-parameter
scaling) that turn it from a theoretical idea into the algorithm that trains essentially
...
**minibatch** $\mathcal{B}$: think 32 examples against a million:

$$
\vect{w}^{(t+1)} = \vect{w}^{(t)} - \alpha \, \frac{1}{|\mathcal{B}|}\sum_{i \in \mathcal{B}} \nabla \ell_i(\vect{w}^{(t)}).
$$ {#eq-sgd}

Why does this work? At a *fixed* parameter vector, a uniformly sampled minibatch
...
There is a second, physical reason to batch. The hardware that trains networks is built
around batched matrix multiplication, the GEMM of the Preface. A dense layer applied to
$B$ examples is one matrix product in which every weight, once read from memory, is
reused for all $B$ examples. For small $B$ the step's cost is dominated by moving
weights rather than by arithmetic, so a batch of 32 often takes little more wall-clock
time than a batch of one, and $B=1$ leaves most of an accelerator idle. Batch size is a
negotiation between statistics and silicon; @sec-04-estimator-cases and the batch-size
section below return to both sides.

In practice the algorithm is:

1. Shuffle the training data.
2. Split it into minibatches of size $B$.
...
Training with SGD moves through two regimes, a pattern often called
**semi-convergence**: rapid, decisive progress early, then persistent oscillation near
the solution. The mechanism is a shifting balance between signal and noise. At any
parameter vector, split the minibatch gradient into the full gradient plus zero-mean
sampling noise; the zero mean is @eq-unbiased, which holds because the batch is drawn
at the same parameter vector where the gradient is evaluated:
...
On our convex least-squares bowl the two regimes are easy to see:

- **Far from the optimum, signal dominates.** The full gradient is large relative to
  the batch noise, so most batch directions make useful progress, and their mean is the
  full descent direction.
- **Near the optimum, noise dominates.** The full gradient shrinks toward zero, but
  individual batches still disagree ("go left", "go right") because different examples
  pull in different directions. With a fixed learning rate the iterate keeps bouncing
...
a shallow trap that exact descent would follow more predictably. Minibatch noise can
...
2. Open one figure for the three loss curves.
3. Implement the learning-rate triptych.
4. Add a legend naming each learning rate and show the figure.
:::
...
lives only here. @sec-a5-statistical-learning places these cases beside the population
quantity, distribution, and uncertainty that complete the statistical contract.
...
The ball is literal physics. A ball of mass $m$ moving through a viscous fluid with drag
coefficient $\gamma$ over the loss landscape obeys

$$
m\,\ddot{\vect{w}} = -\gamma\,\dot{\vect{w}} - \nabla\loss(\vect{w}):
$$

acceleration comes from the downhill force minus friction. Step it forward with time
step $\Delta t$ and write $\Delta\vect{w}^{(t)} = \vect{w}^{(t+1)} - \vect{w}^{(t)}$ for
the displacement:

$$
\Delta\vect{w}^{(t)} = \beta\,\Delta\vect{w}^{(t-1)} - \alpha\,\nabla\loss(\vect{w}^{(t)}),
\qquad
\beta = 1 - \frac{\gamma\,\Delta t}{m},
\quad
\alpha = \frac{\Delta t^{2}}{m}.
$$

This is @eq-momentum with $\Delta\vect{w}^{(t)} = -\alpha\,\vect{v}^{(t)}$. So $\beta$ is
the fraction of velocity that survives one step of friction: $\beta = 0$ is a ball in syrup, which is plain
gradient descent, and $\beta \to 1$ is a frictionless ball that never settles.

Unrolled, the velocity is a geometrically weighted sum of past gradients,
$\vect{v}^{(t)} = \sum_{k \ge 0} \beta^k\,\nabla\loss(\vect{w}^{(t-k)})$, which makes
momentum a low-pass filter. A gradient component that keeps its sign is amplified by
$\sum_k \beta^k = 1/(1-\beta)$, ten times at $\beta = 0.9$; a component that flips sign
every step is scaled by $\sum_k (-\beta)^k = 1/(1+\beta)$, about $0.53$. The steady
downhill signal passes through, while the chatter across the valley and the minibatch
noise, which has no preferred sign from step to step, are averaged down relative to it.
...
:::: {.plan-code .plan-code-wide}
::: {.plan}
1. Fix the seed, draw 256 inputs with feature scales 1 and 10, and form noisy targets
   from `w_true`.
2. Define the reusable `race` helper.
3. Implement the optimizer race.
4. Add a legend naming each optimizer and show the loss curves.
:::
...
### Weight decay: intervening on the update

Under plain SGD, adding an $L_2$ penalty is equivalent (up to whether
...
`weight_decay` argument, but the equivalence stops at plain SGD. Under Adam, the $L_2$
term $\lambda\vect{w}$ joins the gradient, passes through Adam's moment estimates, and
is divided by the coordinate-wise scale $\sqrt{\hat{\vect{s}}^{(t)}} + \epsilon$ of
@eq-adam, so weights with a large gradient history are shrunk less. **AdamW** decouples
the two: it applies $\vect{w} \leftarrow (1 - \alpha\lambda)\vect{w}$ directly to the
parameters, outside the adaptive step.
...
### Dropout: intervening on the architecture

<!-- NOVEL: needs sign-off -->
Weight decay changes the update. Dropout changes the network that receives
...
2. Fix the seed and average 20,000 `inverted_dropout` draws of the four-entry `h` at p =
   0.25.
...
masked-network outputs: nonlinearities do not commute with expectations, and in general
$\E[\sigma(\vect{z})] \ne \sigma(\E[\vect{z}])$.

### Early stopping: intervening on training duration

Track the validation loss during training and stop when it turns
...
<!-- NOVEL: needs sign-off -->
All three are cheap relative to training the model, widely used, and work from inside
the training loop by constraining or randomizing the fit.
...
- Why does an $L_2$ penalty not shrink weights uniformly under Adam, and what does AdamW
  change?
- Why does inverted dropout need no rescaling at evaluation time, and what goes wrong if
  `model.train()` is left active during validation?
:::
...
1. Instantiate the parameters $\vect{w}^{(0)}$: PyTorch layers initialize their weights
   automatically when they are created (@sec-05-backpropagation analyzes why that
   initial scale governs trainability).
2. For each epoch: shuffle, split into minibatches of size $B$.
...
- Tieleman and Hinton, “Lecture 6.5 — RMSProp,” *COURSERA: Neural Networks for
  Machine Learning* (2012): the original public provenance for RMSProp's running
  average of squared gradients.
- Gilbert Strang, [*Linear Algebra and Learning from Data*](https://math.mit.edu/~gs/learningfromdata/),
  §VI.4–VI.5, is a classical numerical-linear-algebra treatment: gradient descent,
  condition numbers, and momentum on quadratic bowls (§VI.4), and stochastic gradient
  descent's semi-convergence, unbiased-gradient assumption, and convergence in
  expectation (§VI.5, especially pp. 361–365).
...
2. **(Code.)** On the two-zones problem, run 100 SGD steps from the far point
   $(-0.5, 2)$ with one fixed learning rate (for example $\alpha = 0.05$), drawing each
   batch *without replacement* from the 80 examples, for $B \in \{1, 4, 16, 80\}$.
   Measure the radius of the region of confusion as the root-mean-square distance of the
   last 30 iterates from the optimum,
   $\bigl(\tfrac{1}{30}\sum_{t=71}^{100}\lVert\vect{w}^{(t)}-\vect{w}^\star\rVert_2^2\bigr)^{1/2}$.
   How does it scale with $B$? Compare against $1/\sqrt{B}$ first, then include the
   finite-population correction $\sqrt{(n-B)/(n-1)}$. Why must the noise vanish at
   $B=80$?
...
5. **(Code.)** Rescale each column of `Xr` to unit standard deviation and rerun all
   three optimizers. The columns are already zero-mean and the model has no intercept,
   so this scaling is full standardization here. Retune SGD's learning rate, which was
   sized for the old steep direction, and compare full-batch losses rather than weights,
   since rescaling changes the parameterization. How much of Adam's advantage
...
6. **(Pencil.)** (a) From @eq-inverted-dropout, show that
   ...
   **(Code.)** (b) Estimate that variance
   ...
   $p \to 1$, and what defect does leaving masking on introduce at evaluation? (Think
   about determinism and variance, not the mean, which inverted dropout already
   preserves.)
