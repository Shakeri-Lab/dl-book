...
### Rolling build · September 4, 2026 · A reproducible reference {.unnumbered .unlisted}

The book's numerical reference now travels with its environment: a saved Linux
container, exact package hashes, and a record of the machine and execution
settings. Two independent runs must agree before new results enter the book.
The downloadable notebooks are checked in that same environment; other supported
runtimes have a separate, measured portability check.

Chapter 11 now compares its two training regimes across five paired seeds, and
Chapter 13 computes its own matching baseline. Variation between seeds informs
the precision of the prose; paired differences between runtimes inform portability
bounds. These are different questions. HTML remains the canonical edition, and
the PDFs remain derived print conversions of the same manuscript and evidence.
