...
and teaching improvements. The current stable edition is **v1.2.1 (August 2, 2026)**;
its [archived release and PDF](https://github.com/Shakeri-Lab/dl-book/releases/tag/v1.2.1)
preserve a fixed version. The live site is a rolling post-v1.2.1 build.
...
Version 1.2.1. [https://shakeri-lab.github.io/dl-book/](https://shakeri-lab.github.io/dl-book/).
:::
...
|---|---|---|---|
| [I · From Lines to Networks](chapters/part1/01-linear-regression.qmd) | the input features | intermediate representations | global templates span the whole frame, producing the shift cliff |
| [II · Vision](chapters/part2/07-filters-convolution.qmd) | hand-designed local filters | detector weights | a one-shot fixed-width code cannot process an arbitrary-length stream |
| [III · Sequences](chapters/part3/10-sequences-rnn.qmd) | a one-shot summary | the carried state update | one fixed-size encoder–decoder bridge must carry every source fact |
| [IV · Attention](chapters/part4/12-kernel-regression.qmd) | the similarity rule | the comparison rule | full fine-tuning copies or updates the whole pretrained backbone per task |
| [V · The Pretrained Era](chapters/part5/17-peft-quantization.qmd) | the reuse rule or sampler | adaptation, a learned judge, and generation | objectives, data, and evaluation remain choices no architecture makes |
...
The Preface now states the book's scope and central bet, maps the five-part
learnability route, and places its reading loop among several standard references.
Cross-volume pointers describe *Deep Learning: Making It Trainable* only as an
optional, self-contained further route, and the HTML audit warns when future prose
implies dependency. A bounded rebuild-and-check loop verifies every derived-PDF
outline destination against its rendered heading. Housekeeping removes temporary
review markers, gives the branded 404 page a keyboard-first skip path, and defines
the visible stamp as the deterministic content-revision date rather than the build date.
...
[`CITATION.cff`](https://github.com/Shakeri-Lab/dl-book/blob/v1.2.1/CITATION.cff).
:::
::::
