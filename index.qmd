...
![](figures/generated/0_0_preface_engine.svg){width=100% fig-alt="An input matrix X passes left to right through layers 1, 2, and L, each holding a weight matrix and bias, to a prediction Y hat. The prediction and the target Y feed the loss. Solid blue arrows along the top carry data forward; dashed dark red arrows along the bottom carry the loss's sensitivities backward to every layer."}

::: {.preface-figure-note}
*Forward, data flows left to right; backward, sensitivities flow right to left. Every layer holds learnable parameters (orange), and the loss compares the prediction (green) with the target (purple).*
:::

- **Forward, left to right.** Data flows through linear maps, one batched GEMM per
  layer, each followed by a nonlinear bend.
- **Backward, right to left.** Reverse-mode differentiation carries the loss's
  sensitivity back to every parameter, one vector–Jacobian product per layer.
...
### Rolling build · September 25, 2026 · Preface, Chapters 1 to 7, and references {.unnumbered .unlisted}
...
a ball with friction. Chapter 5 now develops backpropagation as a recursion on layer
sensitivities, with redrawn figures, a new figure of four autograd rules in PyTorch
syntax, the batch form of the weight gradient, and the cost of the backward pass in
arithmetic and in memory; its BatchNorm exercise moved to Chapter 9, where BatchNorm is
introduced. Chapter 6 completes its two experiments into a two-by-two protocol,
separates cropping from moving in the shift experiment, shows why a dense layer cannot
see pixel adjacency, and cites the theorem behind confident predictions far from the
training data. The experimentation interlude's opening figure now draws both levels of
learning: the training run inside the experiment cycle. Chapter 7 now states directly
why it builds convolution with fixed kernels first and what changes when the kernel is
learned. Across the book, every code panel's plan now names what its steps do.
