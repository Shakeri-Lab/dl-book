...
$$
\underbrace{f(x) = \vect{w}^\top \phi(x)}_{\text{linear model on fixed features}}
\qquad\longrightarrow\qquad
\underbrace{f_\theta(x) = \vect{w}^\top \phi_\theta(x)}_{\text{linear readout on learned nonlinear features}}.
$$
...
\textbf{What is built in?} &\quad \text{locality, sharing, order, invariance, causality} \\
...
experiments show their code on the page, and concept diagrams fold theirs. Source,
provenance, and licenses remain visible in the repository.
...
written directly in Python and PyTorch and runs on an ordinary laptop CPU — small data,
honest experiments. Watch the [lecture videos](https://www.youtube.com/playlist?list=PLwZv_wzaKu3vzW88m6ASKemCTb70ukycH),
