\begin{tikzpicture}[scale=1.0]
  ...
  \draw[thick, red, domain=0.5:6.6, samples=150] plot (\x, {0.02*(\x-0.5)^3 + 0.3});
  \node[red] at (6, 2.8) {Variance};

  % Total error curve (smoother U-shape; domain stops below the region labels)
  \draw[thick, purple, domain=0.5:6.2, samples=150] plot (\x, {2.5*exp(-0.6*\x) + 0.02*(\x-0.5)^3 + 0.7});
  ...
  \node[fill=white, inner sep=1.5pt] at (1.8, 4.8) {\textbf{Underfitting}};
  \draw[<->] (4.2, 4.5) -- (7.2, 4.5);
  \node[fill=white, inner sep=1.5pt] at (5.7, 4.8) {\textbf{Overfitting}};
\end{tikzpicture}
