\begin{tikzpicture}[scale=1.2]
  ...
  \draw[very thick, dlpurple, ->] (0,0) -- (3.2,3.8)
    node[midway, above left=-1pt] {$\targetpart{\mathbf{y}}$};

  % Projection onto column space - more prominent
  \draw[very thick, dlgreen, ->] (0,0) -- (2.4,2.0)
    node[below right=-1pt]
    {$\predictionpart{\hat{\mathbf{y}}}
      = \featurepart{\mathbf{X}}\parameterpart{\hat{\mathbf{w}}}$};
  
  % Residual vector (orthogonal) - more prominent
  \draw[very thick, dlwine, ->] (2.4,2.0) -- (3.2,3.8)
    node[above right]
    {$\residualpart{\mathbf{e}}
      = \targetpart{\mathbf{y}}-\predictionpart{\hat{\mathbf{y}}}$};
  ...
  \node[dlblue, font=\small] at (0.8, 0.8)
    {Column space of $\featurepart{\mathbf{X}}$};
  \node[dlwine, font=\small] at (4.5, 3.0)
    {$\residualpart{\mathbf{e}}
      \perp \operatorname{col}(\featurepart{\mathbf{X}})$};
\end{tikzpicture}
