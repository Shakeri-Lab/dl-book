\begin{tikzpicture}[scale=1.2]
  ...
  \draw[very thick, dlpurple, ->] (0,0) -- (3.2,3.8) node[midway, above left=-1pt] {$\mathbf{y}$};

  % Projection onto column space - more prominent
  \draw[very thick, dlgreen, ->] (0,0) -- (2.4,2.0) node[below right=-1pt] {$\hat{\mathbf{y}} = \mathbf{X}\hat{\mathbf{w}}$};
  
  % Residual vector (orthogonal) - more prominent
  \draw[very thick, dlorange, ->] (2.4,2.0) -- (3.2,3.8) node[above right] {$\mathbf{e} = \mathbf{y} - \hat{\mathbf{y}}$};
  ...
  \node[dlorange, font=\small] at (4.5, 3.0) {$\mathbf{e} \perp \text{col}(\mathbf{X})$};
\end{tikzpicture}
